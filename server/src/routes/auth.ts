...
router.post("/logout", async (req, res) => {
  ...
  if (token) {
    await supabase.from("tokens").delete().eq("refresh_token", token);
  }
...
router.post("/login", async (req, res) => {
  ...
  if (!user) {
    return res
      .status(400)
      .json({ errorCode: "EMAIL_NOT_FOUND", message: "email not found" });
  }
...
router.post("/refresh", async (req, res) => {
  ...
  if (!data) return res.sendStatus(403); // token ถูก revoke แล้ว
...
router.get("/me", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      accessToken: generateAcessToken(decoded.user_id),
...
const getRefreshToken = async (user_id: any) => {
  ...
  const expiredIds =
    tokens?.filter((t) => new Date(t.expires_at) < now).map((t) => t.id) || [];
  ...
  if (tokens && tokens.length > 0) {
    await supabase.from("tokens").delete().eq("user_id", user_id);
  }
